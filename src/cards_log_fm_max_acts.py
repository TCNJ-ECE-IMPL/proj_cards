...
# Defines a TensorFlow Dataset Object, based on stored images
# Images from each class are labeled according to the name their subdirectory
...
# Define the target layer for extraction and logging
LAYER_NAME   = 'conv_4'
...
for batch in full_ds:
    ...
    # batch[1] contains labels, len(batch[1]) tells us the number of images in batch
    for idx in range (len(batch[1].numpy())):

        # The idea is to look at only a fraction of all of the rotations
        # So impose an upper limit for rotation angle of ROT_LIM, and
        # sample one image each ROT_MOD degrees
        ...
        if log_act_flag:
            ...
            # I think activations is a 4D numpy array: <batch_idx><fm_horizontal><fm_vertical><fm_idx>
            # access as [0, :, :, fm_idx]
            
            # instead of outputting,  just save away the maximum activation in each feature map in the current layer

            # Iterate through feature maps

                # For each feature map, extract the max, and log it
...
# at the end, write out the desired table of max. activations